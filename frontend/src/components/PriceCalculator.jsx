...
export default function PriceCalculator() {
  ...
  return (
    <div id="calculator" className="bg-white dark:bg-ink-soft border border-slate-200 dark:border-line rounded-2xl p-6 sm:p-8 scroll-mt-24">
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        <label className="block">
          <span className="block text-sm font-medium text-ink dark:text-paper mb-1.5">Pickup city</span>
        ...
        <label className="block">
          <span className="block text-sm font-medium text-ink dark:text-paper mb-1.5">Drop-off city</span>
        ...
        <label className="block">
          <span className="block text-sm font-medium text-ink dark:text-paper mb-1.5">Vehicle</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-paper border border-slate-200 dark:border-line rounded-xl px-5 py-4">
        <div>
          <div className="text-xs text-slate dark:text-slate-light mb-1">Estimated price</div>
          <div className="font-mono font-semibold text-2xl">
            {loading ? "…" : price !== null ? `₦${price.toLocaleString()}` : "—"}
          </div>
          {!loading && distanceKm !== null && (
            <div className="text-xs text-slate dark:text-slate-light mt-1">≈ {distanceKm} km driving distance</div>
          )}
        </div>
        <Link
          to="/signup/customer"
          className="w-full sm:w-auto text-center bg-route hover:bg-route-dark text-ink dark:text-paper font-semibold rounded-lg px-6 py-3 transition-colors"
        >
          Book this delivery
        </Link>
      </div>
      <p className="text-xs text-slate dark:text-slate-light mt-3">
        {distanceKm !== null
