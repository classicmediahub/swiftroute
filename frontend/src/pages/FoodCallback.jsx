...
export default function FoodCallback() {
  ...
  useEffect(() => {
    ...
    api.verifyFoodPayment(token, reference)
      .then((data) => {
    ...
    <div className="max-w-md mx-auto px-5 py-24 text-center">
      ...
      {status === "paid" && (
        <>
          ...
          <h1 className="font-display text-xl font-semibold text-ink dark:text-paper mb-2">Order confirmed</h1>
      ...
      {status === "failed" && (
        <>
          ...
          <p className="text-sm text-slate dark:text-slate-light mb-6">Nothing was charged. You can try again.</p>
          <Link to="/food" className="text-sm font-semibold text-route-dark hover:underline">Back to food →</Link>
        </>
      )}
      {status === "error" && (
        <>
